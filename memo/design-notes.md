...
其中 `data_requirements` 建议作为可选字段，只允许出现在 scenario / patch 层，用来声明“这条 knowhow 在什么条件下需要额外补拉哪些 CRM 数据”。

建议结构：
- request_group：必须对应 retrieval mapping 中已有的 request group
- fields：必须来自 CRM data dictionary
- why：说明这些字段用于关闭什么业务不确定性
- conditions：说明在什么 meeting / CRM 证据前提下才触发
- priority：high / optional

边界：
- data_requirements 只能细化 allowed request groups 内的字段，不能绕过 retrieval policy 新增默认 scope
- 如果 data_requirements 指向 mapping 之外的 request group，只能进入 out_of_policy_requests，不能默认拉取
- low-confidence / uncertain 模式下，不执行 scenario / patch knowhow 的 data_requirements
